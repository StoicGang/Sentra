...
class Colors:
    ...
    def _wrap(self, text: str, code: str) -> str:
        if not self._enabled:
            return text
        safe_text = str(text).replace("\033", "")
        return f"\033[{code}m{safe_text}\033[0m"
...
class SentraCLI:
    ...
    def _unlock_existing_vault(self) -> bool:
        ...
        while attempt <= MAX_UNLOCK_ATTEMPTS:
            ...
            try:
                self.vault.unlock_vault(pw, create_if_missing=False)
                self.session_active = True
    ...
    def cmd_list(self, args):
        """List entries with sorting and filtering (Keyset Pagination)"""
        if not self.ensure_unlocked():
            return

        # Cursor variables for keyset pagination
        last_ts = None
        last_id = None
        limit = 20
        page_num = 1
        ...
        try:
            while True:
                ...
                # Client-side sorting (only affects current page display in this simple implementation)
                ...
                print(f"\n{colors.info(f'--- {header} (Page {page_num}) ---')}\n")
                ...
                # If fetched fewer than limit, we reached the end
                if fetched_count < limit:
                    print(colors.dim("--- End of entries ---"))
                    break

                # Update cursors for next fetch
                # We assume the DB returns order by modified_at DESC, id DESC
    ...
    def cmd_delete(self, args):
        """Delete entry with safety confirmation (Soft or Hard)"""
        if not self.ensure_unlocked():
            return
        ...
        try:
            # === CHANGE 1: Search Logic ===
            # If deleting permanently, search Trash too (so you can purge deleted items)
            include_trash = args.permanent
            matches = self.vault.search_entries(query, include_deleted=include_trash)
            entry_id = choose_from_list(matches)
            ...
            entry = self.vault.get_password(entry_id, include_deleted=args.permanent)
            if not entry:
                print_error("Entry not found.")
                return

            # === CHANGE 2: Dynamic Warning Message ===
            if args.permanent:
                print(f"\n{colors.error('⛔ PERMANENT DELETE WARNING ⛔')}")
                print(f"  Title: {entry.get('title')}")
                print(f"  This action is {colors.error('IRREVERSIBLE')}. It will be gone forever.")
            ...
            # === CHANGE 3: Execute Action ===
            if args.permanent:
                self.vault.hard_delete_entry(entry_id)
                print_success(f"Entry '{entry.get('title')}' permanently deleted.")
            else:
    ...
    def cmd_import(self, args):
        ...
        if is_csv:
            warning_msg = (
                "⚠️  IMPORTING FROM CSV (PLAIN TEXT)\n"
                "  This will create new entries from the file.\n"
                "  Existing entries with same titles will NOT be overwritten (duplicates may occur)."
            )
        else:
            warning_msg = (
                "⚠️  DANGER: RESTORING BINARY BACKUP\n"
                "  This will OVERWRITE existing entries with matching IDs.\n"
                "  This action cannot be undone."
            )

        print(f"\n{colors.warning(warning_msg)}")
        print()
        ...
        try:
            if is_csv:
                print_info(f"Reading CSV: {filename}...")
                success, failed = self.vault.import_csv(filename)
                print_success(f"✓ Import Complete: {success} imported, {failed} failed.")
            else:
                print_info(f"Restoring Backup: {filename}...")
                # Initialize backup manager dynamically
                bm = self.vault.create_backup_manager()
                bm.restore_backup(filename)
                print_success("✓ Vault successfully restored from backup.")
    ...
    def cmd_security(self, _=None):
        ...
        try:
            ...
            # Weak passwords (strength stored in DB)
            weak_count = sum(
                1 for e in entries
                if (e.get("password_strength") or 100) < 50
            )

            # Old passwords (SQL-based, accurate)
            old_entries = self.vault.get_old_entries(90)
    ...
    def cmd_export(self, args):
        ...
        def sanitize_csv_field(text: str) -> str:
            ...
            # 1. Neutralize Formulas
            if stripped.startswith(forbidden_prefixes):
                # Prepend single quote to force text interpretation
                return "'" + text
            # 2. Neutralize Control Characters (Tab/Newline injection)
            # Use repr() logic or explicit replacement
...
def main():
    ...
    try:
        ...
        try:
            if cli is not None and cli.session_active:
